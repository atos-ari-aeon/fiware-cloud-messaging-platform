...
class aeon_rabbitmq ($rabbitmq_version) {
    ...
    exec { "wget_rabbitmq":
      command => "wget https://www.rabbitmq.com/releases/rabbitmq-server/v${rabbitmq_version}/rabbitmq-server_${rabbitmq_version}-1_all.deb",
      path => ["/bin", "/usr/bin"],
      require => [Exec["preinstallerlang"],Exec["aptGetUpdate"], Package["git"], Package["curl"], Package["g++"]]
    }
    
    exec { "install_dpkg_rabbitmq":
      command => "sudo dpkg -i /home/vagrant/rabbitmq-server_${rabbitmq_version}-1_all.deb",
      path => ["/bin", "/usr/bin"],
